...
本文档介绍使用 zhouhonglin-agent 的最佳实践和建议。
...
```python
import pytest
from src.zhouhonglin_agent.tools.basic_tools import calculate
...
def test_agent_with_mock():
    ...
    with patch('src.zhouhonglin_agent.gitee_ai_client.requests.post') as mock_post:
        # 设置 Mock 响应
        mock_response = Mock()
