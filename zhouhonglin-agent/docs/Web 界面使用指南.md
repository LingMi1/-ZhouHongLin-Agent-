...
zhouhonglin Agent 提供了一个现代化的 Web 界面，让您可以通过浏览器与 Agent 进行交互，无需在控制台运行代码。
...
# 或直接运行
python -m uvicorn zhouhonglin_agent.web_app:app --reload
```
...
在 `src/zhouhonglin_agent/web_app.py` 中的 `get_agent` 函数里修改默认的 `system_message`。

### 添加新工具

如果使用 Tool Agent，可以在 `src/zhouhonglin_agent/tools/basic_tools.py` 中添加新的工具函数。
...
gunicorn zhouhonglin_agent.web_app:app -w 4 -k uvicorn.workers.UvicornWorker --bind 0.0.0.0:8000
```
...
```bash
docker build -t zhouhonglin-agent .
docker run -p 8000:8000 --env-file .env zhouhonglin-agent
```
