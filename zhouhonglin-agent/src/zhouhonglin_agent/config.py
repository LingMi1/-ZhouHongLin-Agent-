...
class Settings(BaseSettings):
    ...
    use_cloud_embedding: bool = Field(
        default=True,
        description="是否使用云端嵌入服务（默认启用，失败后自动降级到本地）"
    )
    cloud_embedding_model: str = Field(
        default="bge-large-zh-v1.5",
        description="云端嵌入模型名称"
    )
    enable_embedding_fallback: bool = Field(
        default=True,
        description="云端嵌入失败时是否自动降级到本地模型（推荐开启，彻底解决余额不足问题）"
    )
    ...
    # 本地嵌入模型配置（仅当 use_cloud_embedding=False 或降级时使用）
    embedding_model: str = Field(
        default="BAAI/bge-small-zh-v1.5",
        description="本地嵌入模型名称（推荐使用 bge-small-zh-v1.5，轻量快速）"
    )
    ...
    similarity_threshold: float = Field(
        default=0.0,
        description="相似度阈值，低于此阈值的片段不返回 (0-1)，设为0禁用过滤"
    )
    ...
    use_local_reranker: bool = Field(
        default=False,
        description="是否使用本地重排序模型（需要下载模型文件，启动较慢）"
    )
    reranker_model: str = Field(
        default="BAAI/bge-reranker-base",
        description="本地重排序模型名称（仅当 use_cloud_reranker=False 且 use_local_reranker=True 时使用）"
    )
