...
class ContextManager:
    ...
    def truncate_text(
        ...
        if last_safe != -1 and last_safe > len(result) - 50:
            result = result[:last_safe + 1]
    ...
    def format_documents_for_prompt(
        ...
        # 转义特殊字符，确保不会破坏JSON结构
        # 注意：这里不需要转义，因为最终会通过json.dumps序列化
        # 但需要确保文本中没有可能导致问题的控制字符
...
请回答："""
    ...
    def _sanitize_text(self, text: str) -> str:
        ...
        text = text.replace('\x08', '')  # BS
        text = text.replace('\x0b', '')  # VT
        text = text.replace('\x0c', '')  # FF
        text = text.replace('\x0e', '')  # SO
        text = text.replace('\x0f', '')  # SI
        text = text.replace('\x10', '')  # DLE
        text = text.replace('\x11', '')  # DC1
        text = text.replace('\x12', '')  # DC2
        text = text.replace('\x13', '')  # DC3
        text = text.replace('\x14', '')  # DC4
        text = text.replace('\x15', '')  # NAK
        text = text.replace('\x16', '')  # SYN
        text = text.replace('\x17', '')  # ETB
        text = text.replace('\x18', '')  # CAN
        text = text.replace('\x19', '')  # EM
        text = text.replace('\x1a', '')  # SUB
        text = text.replace('\x1b', '')  # ESC
        text = text.replace('\x1c', '')  # FS
        text = text.replace('\x1d', '')  # GS
        text = text.replace('\x1e', '')  # RS
        text = text.replace('\x1f', '')  # US
