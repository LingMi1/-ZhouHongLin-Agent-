...
class DocumentLoader:
    ...
    def split_text(
        ...
        Returns:
            分片后的文档列表
        """
        print(f"[split_text] 输入文本长度: {len(text)}")
        
        # 如果文本太短（小于 chunk_size），直接返回整个文本作为单个 chunk
        if len(text) < 50:
            print(f"[split_text] 文本太短，直接作为单个片段返回")
            documents = [Document(page_content=text, metadata=metadata or {})]
            return documents
        
        chunks = self.text_splitter.split_text(text)
        print(f"[split_text] 切分后片段数: {len(chunks)}")
        
        # 如果切分结果为空，至少返回原始文本
        if not chunks:
            print(f"[split_text] 切分结果为空，返回原始文本")
            documents = [Document(page_content=text, metadata=metadata or {})]
            return documents
        ...
        print(f"[split_text] 最终返回 {len(documents)} 个有效片段")
        return documents
